...
- ❌ OpenNext - falha no build
- ❌ Deploy manual - retorna 404
- ❌ @cloudflare/next-on-pages - não funciona no Windows
- ❌ Export estático - projeto tem API routes (incompatível)
- ✅ **Build automático do Cloudflare - FUNCIONA!** (build completa, só precisa ajustar output directory)
...
Então:
- ❌ `apps/web/.next` = ERRADO (procura em apps/web/apps/web/.next)
- ✅ `.next` = CERTO (procura em apps/web/.next)
